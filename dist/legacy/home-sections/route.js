...
async function GET() {
    try {
        ...
        for (const section of sections) {
            ...
            if (section.shuffleIntervalMinutes > 0 &&
                ...
                try {
                    ...
                    const newOrder = await (0, groq_1.shuffleProductsWithGroq)(productInfos);
                    ...
                    const freshProducts = await prisma_1.prisma.homeSectionProduct.findMany({
                        where: { sectionId: section.id },
                        include: {
                            product: {
                                select: {
        ...
        const result = sections.map(section => ({
            ...
            products: section.products.map(sp => ({
                ...sp.product,
                discount: sp.product.mrp > sp.product.sellPrice
        ...
        return server_1.NextResponse.json({ error: "Failed to fetch sections" }, { status: 500 });
    }
}
