...
async function GET(req) {
    try {
        ...
        const targetDate = dateParam ? (0, date_fns_1.parseISO)(dateParam) : new Date();
        ...
        const deliveryBoys = await prisma_1.prisma.user.findMany({
        ...
        const shippedOrders = await prisma_1.prisma.order.findMany({
            where: {
        ...
        const dailyOrders = await prisma_1.prisma.order.findMany({
            where: {
                deliveryCodeId: { in: codeIds },
                updatedAt: {
                    gte: dayStart,
