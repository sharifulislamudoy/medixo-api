...
async function POST(req) {
    try {
        ...
        if (!cutoff) {
            return server_1.NextResponse.json({ error: 'Cutoff time required' }, { status: 400 });
        }
 ...
 */
async function mergeProcessingOrders(tx, orders) {
    ...
    for (const [phone, group] of groups.entries()) {
        ...
        const target = group.reduce((best, current) => {
            if (current.totalAmount > best.totalAmount)
                return current;
