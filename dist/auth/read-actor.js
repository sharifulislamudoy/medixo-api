...
async function readActor(req) {
    ...
    const decoded = await (0, jwt_1.decode)({ token, secret: process.env.NEXTAUTH_SECRET });
    if (!decoded?.id || !decoded.sessionId)
        throw new common_1.UnauthorizedException("Session expired");
    const [user, session] = await Promise.all([
        prisma_1.prisma.user.findUnique({ where: { id: String(decoded.id) }, select: { id: true, role: true, status: true, name: true, email: true } }),
        prisma_1.prisma.session.findUnique({ where: { token: String(decoded.sessionId) }, select: { userId: true } }),
    ]);
