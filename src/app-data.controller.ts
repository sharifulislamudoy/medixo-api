...
async function requireRole(request: Request, role: "SHOP_OWNER" | "DELIVERY_BOY" | "ADMIN") {
  const actor = await readActor(request);
  if (actor.role !== role) throw new UnauthorizedException(`${role} access required`);
  return actor;
}
...
export class AppDataController {
  @Get("customer/new-arrivals")
  async newArrivals(@Req() request: Request) {
  ...
  @Get("customer/advertisements/:slug")
  async announcement(@Req() request: Request, @Param("slug") slug: string) {
    await requireRole(request, "SHOP_OWNER");
  ...
  async product(@Req() request: Request, @Param("slug") slug: string) {
    await requireRole(request, "SHOP_OWNER");
    const product = await prisma.product.findFirst({
      where: { slug, status: true },
      select: publicProductSelect,
    });
    if (!product) return { product: null, similar: [], suggested: [] };
    ...
    const byDiscount = (items: typeof similar) =>
      items.sort((a, b) =>
        (b.mrp > b.sellPrice ? (b.mrp - b.sellPrice) / b.mrp : 0) -
        (a.mrp > a.sellPrice ? (a.mrp - a.sellPrice) / a.mrp : 0),
      );
    return { product, similar: byDiscount(similar), suggested: byDiscount(suggested) };
  }

  @Get("customer/sitemap-products")
  async sitemapProducts(@Req() request: Request) {
    await requireRole(request, "SHOP_OWNER");
    return prisma.product.findMany({
      where: { status: true },
  ...
  async reviews(@Req() request: Request) {
    await requireRole(request, "ADMIN");
  ...
  async assignedOrders(@Req() request: Request) {
    const actor = await requireRole(request, "DELIVERY_BOY");
    const user = await prisma.user.findUnique({
      where: { id: actor.id },
      select: { deliveryCodeId: true },
    });
    if (!user?.deliveryCodeId) return { assigned: false, orders: [] };
  ...
  @Get("delivery/assigned/orders/:id")
  async assignedOrder(@Req() request: Request, @Param("id") id: string) {
    const actor = await requireRole(request, "DELIVERY_BOY");
    const user = await prisma.user.findUnique({
      where: { id: actor.id },
      select: { deliveryCodeId: true },
    });
    if (!user?.deliveryCodeId) return { assigned: false, order: null };
  ...
  @Get("delivery/assigned/cash")
  async cash(@Req() request: Request) {
    const actor = await requireRole(request, "DELIVERY_BOY");
    const user = await prisma.user.findUnique({
      where: { id: actor.id },
      select: { deliveryCodeId: true },
    });
    if (!user?.deliveryCodeId) return { assigned: false };
    ...
    let returnedTotal = 0;
    const returnedItemsMap = new Map<string, {
      productName: string;
      productImage: string;
    ...
    for (const order of orders) {
      const returnedValue = order.items.reduce(
        (sum, item) => sum + item.returnedQuantity * item.price, 0,
      );
      if (returnedValue > 0) {
      ...
      if (order.status === "SHIPPED") assignedTotal += order.totalAmount;
  ...
  async history(@Req() request: Request) {
    ...
    if (!user?.deliveryCodeId) return { assigned: false, history: [] };
    const orders = await prisma.order.findMany({
      where: {
        deliveryCodeId: user.deliveryCodeId,
    ...
    for (const order of orders) {
      ...
      entry.returnedAmount += order.items.reduce(
        (sum, item) => sum + item.returnedQuantity * item.price, 0,
      );
  ...
  @Get("delivery/leaderboard")
  async leaderboard(@Req() request: Request) {
  ...
  @Get("admin/user-activities/list")
  async userActivities(@Req() request: Request) {
    await requireRole(request, "ADMIN");
